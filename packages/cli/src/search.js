...
export async function searchAssets(query,{catalog=defaultCatalog,pack,source,type,availability='ready',limit=10,offset=0}={}) {
  ...
  if (!['ready','source','all'].includes(availability)) throw new Error('--availability must be ready, source or all');
  const remote=/^https:\/\//.test(catalog);
  const sourceCatalog=remote?new URL('source-catalog.json',catalog).href:resolve(dirname(catalog),'source-catalog.json');
  const catalogs=availability==='ready'?[catalog]:availability==='source'?[sourceCatalog]:[catalog,sourceCatalog];
  const lists=await Promise.allSettled(catalogs.map(async path=>{
    let data;if(remote){const response=await fetch(path,{signal:AbortSignal.timeout(30000)});if(!response.ok)throw new Error(`Catalog request failed: HTTP ${response.status}`);data=await response.json();}
    else data=JSON.parse(await readFile(path,'utf8'));
  ...
  const records=[];for(const list of lists){if(list.status==='rejected')throw list.reason;records.push(...list.value);}
  const terms=words(query);
  const hits=[];
  const seen=new Set();
  for (const asset of records) {
    const ready=(asset.availability??'ready')==='ready';
    if (typeof asset.id!=='string' || typeof asset.pack!=='string' || typeof asset.label!=='string' || typeof asset.description!=='string') throw new Error('Malformed asset in catalog');
    if(ready&&!['model','thumbnail','descriptor'].every(key=>safePath(asset[key])))throw new Error('Malformed ready asset paths');
    if(!ready&&(asset.availability!=='source'||asset.model!==null||asset.descriptor!==null||![asset.thumbnail,asset.source?.listing,asset.source?.downloadManifest].every(url=>typeof url==='string'&&/^https:\/\//.test(url))))throw new Error('Malformed source listing');
    if(availability==='ready'&&!ready||availability==='source'&&ready)continue;
    if(seen.has(asset.id))continue;seen.add(asset.id);
    ...
    hits.push({asset:{...asset,creator,type:assetType,availability:ready?'ready':'source'},score});
  }
  hits.sort((a,b)=>b.score-a.score || a.asset.id.localeCompare(b.asset.id));
  const results=hits.slice(offset,offset+limit).map(({asset})=>({
    ...asset,
    ...(asset.availability==='ready'?Object.fromEntries(['model','thumbnail','descriptor'].map(key=>[key,remote ? new URL(asset[key],catalog).href : resolve(dirname(catalog),asset[key])])):{}),
  }));
  return {query,pack:pack??null,source:source??null,type:type??null,availability,catalog:catalogs[0],catalogs,total:hits.length,offset,count:results.length,results};
}
export async function searchCommand(args) {
  ...
  try {
    for(let i=0;i<args.length;i++) {
      ...
      if (['--pack','--source','--type','--availability','--limit','--offset','--catalog'].includes(argument)) {
        const value=args[++i];
